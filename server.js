...
const datosIniciales = {
  concursos: [
    ...
    { id: "c2",  lugar: "SANTA",         colegio: "I.E.P. G.M.L.J. ORBEGOSO",         docente: "Erik Obando",     fechaTxt: "23 de Agosto, 2026",    fechaISO: "2026-08-23T09:00:00",  logo: "orbegoso.png",              estado: "finalizado" },
    { id: "c3",  lugar: "NUEVO CHIMBOTE",colegio: "I.E. ABELARDO QUIÑONES",           docente: "Ernesto Lomparte", fechaTxt: "29 de Agosto, 2026",    fechaISO: "2026-08-29T09:00:00", logo: "quinones.png",              estado: "finalizado" },
    ...
    { id: "c5",  lugar: "CASMA",         colegio: "I.E. REPÚBLICA DE CHILE",          docente: "Ernesto Lomparte", fechaTxt: "27 de Septiembre, 2026",fechaISO: "2026-09-27T09:00:00", logo: "chile.png",                 estado: "pendiente" },
  ...
  colegios: [
    ...
    { id: "C021", nombre: "I.E.P. SCHOOL KINDER KING", cat: "B", logo: "kinder.png", resultados: {} },
...
// Endpoint para guardar cambios localmente y enviarlos a GitHub para autosave
app.post('/api/guardar', async (req, res) => {
  ...
  try {
    ...
    console.error("Error al escribir localmente en data.json:", err);
  }

  // 2. Intentar guardar en GitHub para persistencia en Railway
  try {
    if (process.env.GITHUB_TOKEN && process.env.GITHUB_REPO) {
      const [owner, repo] = process.env.GITHUB_REPO.split('/');
      const branch = process.env.GITHUB_BRANCH || 'main';
      const pathArchivo = 'data.json';
      ...
      try {
        ...
        console.log("data.json no existe aún en GitHub, se creará uno nuevo.");
      }
      ...
      await octokit.repos.createOrUpdateFileContents({
        ...
        repo,
        path: pathArchivo,
        message: 'feat(data): actualización de datos por el Fundador',
        content: contenidoBase64,
        sha: shaExistente,
        branch: branch,
      });

      return res.json({ ok: true, message: "Datos guardados localmente y sincronizados con GitHub correctamente." });
    } else {
      return res.json({ ok: true, message: "Datos guardados localmente (Variables de GitHub no detectadas)." });
    }
  } catch (error) {
    console.error('Error al sincronizar con GitHub:', error);
    return res.status(500).json({ ok: false, message: 'Se guardó en local pero falló la sincronización con GitHub: ' + error.message });
  }
});
